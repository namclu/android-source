...
class Dog {
	...
	/************************************************/
	void play(){
		if((getWeight() - WEIGHT_LOSS) > MIN_WEIGHT){
			setWeight(-WEIGHT_LOSS);
		}
		mTimesPlayed++;
		if((mTimesPlayed % 6) == 0){
	...
	/************************************************/
	void cutHair(){
			if((getHairLength() - HAIR_CUT_LENGTH) > 0f){
				setHairLength(-HAIR_CUT_LENGTH);
			}
	}
}
